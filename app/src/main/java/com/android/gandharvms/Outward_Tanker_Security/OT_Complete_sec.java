...
public class OT_Complete_sec extends AppCompatActivity {

    int scrollX = 0;
    List<Common_Outward_model> clubList = new ArrayList<>();
    RecyclerView rvClub;
    HorizontalScrollView headerscroll;
    Adapter_OT__Complete_sec adapterOt__completeSec;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imgBtnExportToExcel = findViewById(R.id.btn_otsecExportToExcel);
                ...
                new DatePickerDialog.OnDateSetListener() {
                    ...
                    public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
                        ...
                        if ((isFromDate || !isFromDate)) {
                            ...
                            Toasty.warning(OT_Complete_sec.this, "Invalid date selection", Toast.LENGTH_SHORT).show();
                        }
                    }
                },
                year, month, day);
    ...
    private void exportToExcel(List<Common_Outward_model> datalist) {
        try {
            Sheet sheet = hssfWorkBook.createSheet("OutwardTankersecurity");
            // Create header row
            ...
            headerRow.createCell(5).setCellValue("KL");
            headerRow.createCell(6).setCellValue("TRANSPORTER");
            headerRow.createCell(7).setCellValue("PLACE");
            headerRow.createCell(8).setCellValue("MOBILE NUMBER");
            headerRow.createCell(9).setCellValue("REMARK");
            headerRow.createCell(10).setCellValue("VEHICLE PERMIT");
            headerRow.createCell(11).setCellValue("PUC");
            headerRow.createCell(12).setCellValue("INSURANCE");
            headerRow.createCell(13).setCellValue("VEH FIT CERTIFICATE");
            headerRow.createCell(14).setCellValue("DRIVER-LIC");
            headerRow.createCell(15).setCellValue("RC BOOK");
            ...
            for (int i = 0; i < datalist.size(); i++) {
                ...
                dataRow.createCell(5).setCellValue(dataItem.getKl());
                dataRow.createCell(6).setCellValue(dataItem.getTransportName());
                dataRow.createCell(7).setCellValue(dataItem.getPlace());
                dataRow.createCell(8).setCellValue(dataItem.getMobileNumber());
                dataRow.createCell(9).setCellValue(dataItem.getRemark());
                dataRow.createCell(10).setCellValue(dataItem.getVehiclePermit());
                dataRow.createCell(11).setCellValue(dataItem.getPuc());
                dataRow.createCell(12).setCellValue(dataItem.getInsurance());
                dataRow.createCell(13).setCellValue(dataItem.getVehicleFitnessCertificate());
                dataRow.createCell(14).setCellValue(dataItem.getDriverLicenses());
                dataRow.createCell(15).setCellValue(dataItem.getRcBook());
            }
            // Save the workbook
    ...
    private void saveWorkbookToFile(HSSFWorkbook hssfWorkBook) {
        try {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.R) {
                ...
                int counter = 1;
                String fileName = "Outward Tanker Security Data_" + dateTimeSuffix + ".xls";
                File outputfile = new File(storageVolume.getDirectory().getPath() + "/Download/" + fileName);
                while (outputfile.exists()) {
                    counter++;
                    fileName = "Outward Tanker Security Data_" + dateTimeSuffix + "_" + counter + ".xls";
    ...
    {
        rvClub = findViewById(R.id.recyclerviewitinweighcogrid_tankerin);
        headerscroll = findViewById(R.id.itincoheaderscroll_tankersec);
    }

    private void setUpRecyclerView()
    {
        adapterOt__completeSec  = new Adapter_OT__Complete_sec(clubList);
        FixedGridLayoutManager manager = new FixedGridLayoutManager();
        manager.setTotalColumnCount(1);
        rvClub.setLayoutManager(manager);
        rvClub.setAdapter(adapterOt__completeSec);
    ...
    public void fetchDataFromApiforweigh(String FromDate,String Todate,String vehicleType,char nextprocess, char inOut) {
        ...
        Call<List<Common_Outward_model>> call = outwardTanker.get_tankerin_security_complete(FromDate,Todate,vehicleType,nextprocess,inOut);
        call.enqueue(new Callback<List<Common_Outward_model>>() {
            ...
            public void onFailure(Call<List<Common_Outward_model>> call, Throwable t) {
                ...
                Toasty.error(OT_Complete_sec.this,"failed..!", Toast.LENGTH_SHORT).show();
            }
        });
    }
}