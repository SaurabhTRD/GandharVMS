...
public class SecOut_OR_Complete extends AppCompatActivity {
    ...
    private void exportToExcel(List<Common_Outward_model> datalist) {
        try {
            ...
            headerRow.createCell(5).setCellValue("PARTY NAME");
            headerRow.createCell(6).setCellValue("NET WEIGHT");
            headerRow.createCell(7).setCellValue("SIGN");
            headerRow.createCell(8).setCellValue("DESCRIPTION OF GOODS");
            ...
            for (int i = 0; i < datalist.size(); i++) {
                ...
                dataRow.createCell(7).setCellValue(dataItem.getSignature());
                dataRow.createCell(8).setCellValue(dataItem.getDescriptionofGoods());
    ...
    private void saveWorkbookToFile(HSSFWorkbook hssfWorkBook) {
        try {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.R) {
                ...
                int counter = 1;
                String fileName = "Outward Truck Out Security Data_" + dateTimeSuffix + ".xls";
                File outputfile = new File(storageVolume.getDirectory().getPath() + "/Download/" + fileName);
                while (outputfile.exists()) {
                    counter++;
                    fileName = "Outward Truck Out Security Data_" + dateTimeSuffix + "_" + counter + ".xls";
