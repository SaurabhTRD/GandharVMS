...
import com.android.gandharvms.Outward_Tanker_Production_forms.OT_Completed_bulkload_production;
...
public class OT_Completd_bilkload_laboratory extends AppCompatActivity {
    int scrollX = 0;
    List<Common_Outward_model> clubList = new ArrayList<>();
    RecyclerView rvClub;
    HorizontalScrollView headerscroll;
    Adapter_OT_completed_bulkload_laboratory adapterOtCompletedBulkloadLaboratory;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ot_completd_bilkload_laboratory);
        ...
        imgBtnExportToExcel = findViewById(R.id.btn_bulkloadlabExportToExcel);
                ...
                new DatePickerDialog.OnDateSetListener() {
                    ...
                    public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
                        ...
                        if ((isFromDate || !isFromDate)) {
                            ...
                            Toasty.warning(OT_Completd_bilkload_laboratory.this, "Invalid date selection", Toast.LENGTH_SHORT).show();
                        }
                    }
                },
                year, month, day);
    ...
    private void exportToExcel(List<Common_Outward_model> datalist) {
        try {
            Sheet sheet = hssfWorkBook.createSheet("OutwardTrucksecurity");
            // Create header row
            ...
            headerRow.createCell(5).setCellValue("DENSITY 29.5 C");
            headerRow.createCell(6).setCellValue("BATCH NO");
            headerRow.createCell(7).setCellValue("REMARK");
            ...
            for (int i = 0; i < datalist.size(); i++) {
                ...
                int intimelength = dataItem.getLabInTime()!=null ? dataItem.getLabInTime().length() : 0;
                int outtimelength = dataItem.getLabOutTime()!=null ? dataItem.getLabOutTime().length() : 0;
                ...
                if(intimelength>0)
                {
                    dataRow.createCell(3).setCellValue(dataItem.getLabInTime().substring(12,intimelength));
                }
                if(intimelength>0)
                {
                    dataRow.createCell(4).setCellValue(dataItem.getLabOutTime().substring(12,intimelength));
                }
                dataRow.createCell(5).setCellValue(dataItem.getDensity_29_5C());
                dataRow.createCell(6).setCellValue(dataItem.getBatchNo());
                dataRow.createCell(7).setCellValue(dataItem.getLabRemark());
    ...
    private void saveWorkbookToFile(HSSFWorkbook hssfWorkBook) {
        try {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.R) {
                ...
                int counter = 1;
                String fileName = "Outward Tanker Laboratory(Bulk Load) Data_" + dateTimeSuffix + ".xls";
                File outputfile = new File(storageVolume.getDirectory().getPath() + "/Download/" + fileName);
                while (outputfile.exists()) {
                    counter++;
                    fileName = "Outward Truck Security Data_" + dateTimeSuffix + "_" + counter + ".xls";
    ...
    {
        rvClub = findViewById(R.id.recyclerviewitin_bulkloadlab_cogrid_tankerkin);
        headerscroll = findViewById(R.id.itin_bulkloadlab_coheaderscroll_tankerin);
    }

    private void setUpRecyclerView()
    {
        adapterOtCompletedBulkloadLaboratory  = new Adapter_OT_completed_bulkload_laboratory(clubList);
        FixedGridLayoutManager manager = new FixedGridLayoutManager();
        manager.setTotalColumnCount(1);
        rvClub.setLayoutManager(manager);
        rvClub.setAdapter(adapterOtCompletedBulkloadLaboratory);
    ...
    public void fetchDataFromApiforweigh(String FromDate,String Todate,String vehicleType,char nextprocess, char inOut) {
        ...
        call.enqueue(new Callback<List<Common_Outward_model>>() {
            ...
            public void onFailure(Call<List<Common_Outward_model>> call, Throwable t) {
                ...
                Toasty.error(OT_Completd_bilkload_laboratory.this,"failed..!", Toast.LENGTH_SHORT).show();
            }
        });
    }
}