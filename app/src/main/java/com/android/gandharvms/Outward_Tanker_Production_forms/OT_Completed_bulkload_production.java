...
public class OT_Completed_bulkload_production extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imgBtnExportToExcel = findViewById(R.id.btn_tankerprodcutionbulkExportToExcel);
    ...
    private void exportToExcel(List<Common_Outward_model> datalist) {
        try {
            Sheet sheet = hssfWorkBook.createSheet("OutwardTankerProductionbulk");
            // Create header row
            ...
            headerRow.createCell(7).setCellValue("DESTINATION");
            headerRow.createCell(8).setCellValue("QTY KL");
            headerRow.createCell(9).setCellValue("TRANSPORTER");
            headerRow.createCell(10).setCellValue("NAME OF OFFICER");
            ...
            for (int i = 0; i < datalist.size(); i++) {
                ...
                int intimelength = dataItem.getProInTime()!=null ? dataItem.getProInTime().length() : 0;
                int outtimelength = dataItem.getProOutTime()!=null ? dataItem.getProOutTime().length() : 0;
                ...
                if(intimelength>0)
                {
                    dataRow.createCell(3).setCellValue(dataItem.getProInTime().substring(12,intimelength));
                }
                if(intimelength>0)
                {
                    dataRow.createCell(4).setCellValue(dataItem.getProOutTime().substring(12,intimelength));
                }
                dataRow.createCell(5).setCellValue(dataItem.getOAnumber());
                dataRow.createCell(6).setCellValue(dataItem.getProductName());
                dataRow.createCell(7).setCellValue(dataItem.getLocation());
                dataRow.createCell(8).setCellValue(dataItem.getHowMuchQuantityFilled());
                dataRow.createCell(9).setCellValue(dataItem.getTransportName());
                dataRow.createCell(10).setCellValue(dataItem.getPsign());
                dataRow.createCell(11).setCellValue(dataItem.getProRemark());
            }
            // Save the workbook
    ...
    private void saveWorkbookToFile(HSSFWorkbook hssfWorkBook) {
        try {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.R) {
                ...
                int counter = 1;
                String fileName = "Outward Tanker Production (Bulk load) Data_" + dateTimeSuffix + ".xls";
