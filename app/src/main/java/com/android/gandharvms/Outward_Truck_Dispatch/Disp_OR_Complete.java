...
public class Disp_OR_Complete extends AppCompatActivity {

    int scrollX = 0;
    List<Common_Outward_model> clubList = new ArrayList<>();
    RecyclerView rvClub;
    HorizontalScrollView headerscroll;
    Adapter_Disp_completed adapterDispCompleted;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_disp_or_complete);
        ...
        imgBtnExportToExcel = findViewById(R.id.btn_orindesExportToExcel);
                ...
                new DatePickerDialog.OnDateSetListener() {
                    ...
                    public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
                        ...
                        if ((isFromDate || !isFromDate)) {
                            ...
                            Toasty.warning(Disp_OR_Complete.this, "Invalid date selection", Toast.LENGTH_SHORT).show();
                        }
                    }
                },
                year, month, day);
    ...
    private void exportToExcel(List<Common_Outward_model> datalist) {
        try {
            Sheet sheet = hssfWorkBook.createSheet("OutwardTruckDespatch");
            // Create header row
            ...
            headerRow.createCell(6).setCellValue("OA NUMBER");
            headerRow.createCell(7).setCellValue("QTY");
            headerRow.createCell(8).setCellValue("OTHER OPTION");
            headerRow.createCell(9).setCellValue("SIGN");
            ...
            for (int i = 0; i < datalist.size(); i++) {
                ...
                int intimelength = dataItem.getDespatchInTime()!=null ? dataItem.getDespatchInTime().length() : 0;
                int outtimelength = dataItem.getDespatchOutTime()!=null ? dataItem.getDespatchOutTime().length() : 0;
                ...
                if(intimelength>0)
                {
                    dataRow.createCell(3).setCellValue(dataItem.getDespatchInTime().substring(12,intimelength));
                }
                if(intimelength>0)
                {
                    dataRow.createCell(4).setCellValue(dataItem.getDespatchOutTime().substring(12,intimelength));
                }
                dataRow.createCell(5).setCellValue(dataItem.getTransportName());
                dataRow.createCell(6).setCellValue(dataItem.getOAnumber());
                dataRow.createCell(7).setCellValue(dataItem.getHowMuchQuantityFilled());
                dataRow.createCell(8).setCellValue(dataItem.getDespatchOther());
                dataRow.createCell(9).setCellValue(dataItem.getDespatch_Sign());
                dataRow.createCell(10).setCellValue(dataItem.getDespatchRemark());
            }
            // Save the workbook
    ...
    private void saveWorkbookToFile(HSSFWorkbook hssfWorkBook) {
        try {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.R) {
                ...
                int counter = 1;
                String fileName = "Outward Dispatch Security Data_" + dateTimeSuffix + ".xls";
                File outputfile = new File(storageVolume.getDirectory().getPath() + "/Download/" + fileName);
                while (outputfile.exists()) {
                    counter++;
                    fileName = "Outward Truck Dispatch Data_" + dateTimeSuffix + "_" + counter + ".xls";
    ...
    {
        rvClub = findViewById(R.id.recyclerviewitin_Desp_hcogrid_truckin);
        headerscroll = findViewById(R.id.itin_Desp_coheaderscroll_truckwe);
    }
    private void setUpRecyclerView()
    {
        adapterDispCompleted  = new Adapter_Disp_completed(clubList);
        FixedGridLayoutManager manager = new FixedGridLayoutManager();
        manager.setTotalColumnCount(1);
        rvClub.setLayoutManager(manager);
        rvClub.setAdapter(adapterDispCompleted);
    ...
    public void fetchDataFromApiforweigh(String FromDate,String Todate,String vehicleType,char nextprocess, char inOut) {
        ...
        Call<List<Common_Outward_model>> call = outwardTanker.gettruckdespatchcomplete(FromDate,Todate,vehicleType,nextprocess,inOut);
        call.enqueue(new Callback<List<Common_Outward_model>>() {
            ...
            public void onFailure(Call<List<Common_Outward_model>> call, Throwable t) {
                ...
                Toasty.error(Disp_OR_Complete.this,"failed..!", Toast.LENGTH_SHORT).show();
            }
        });
    }
}