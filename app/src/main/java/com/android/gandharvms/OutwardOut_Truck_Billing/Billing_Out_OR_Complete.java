...
public class Billing_Out_OR_Complete extends AppCompatActivity {

    int scrollX = 0;
    List<Common_Outward_model> clubList = new ArrayList<>();
    RecyclerView rvClub;
    HorizontalScrollView headerscroll;
    Adapter_Billing_Out_Complete adapterBillingOutComplete;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_billing_out_or_complete);
        ...
        imgBtnExportToExcel = findViewById(R.id.btn_oroutbillingExportToExcel);
                ...
                new DatePickerDialog.OnDateSetListener() {
                    ...
                    public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
                        ...
                        if ((isFromDate || !isFromDate)) {
                            ...
                            Toasty.warning(Billing_Out_OR_Complete.this, "Invalid date selection", Toast.LENGTH_SHORT).show();
                        }
                    }
                },
                year, month, day);
    ...
    private void exportToExcel(List<Common_Outward_model> datalist) {
        try {
            Sheet sheet = hssfWorkBook.createSheet("OutwardTruckOutBilling");
            // Create header row
            ...
            headerRow.createCell(5).setCellValue("OA NUMBER");
            headerRow.createCell(6).setCellValue("TRANSPORTER");
            headerRow.createCell(7).setCellValue("GROSS WEIGHT");
            headerRow.createCell(8).setCellValue("TARE WEIGHT");
            headerRow.createCell(9).setCellValue("NET WEIGHT");
            headerRow.createCell(10).setCellValue("SEAL NUMBER");
            headerRow.createCell(11).setCellValue("TOTAL QTY");
            headerRow.createCell(12).setCellValue("INVOICE NUMBER");
            headerRow.createCell(13).setCellValue("BATCH NUMBER");
            ...
            for (int i = 0; i < datalist.size(); i++) {
                ...
                int intimelength = dataItem.getBillingInTime()!=null ? dataItem.getBillingInTime().length() : 0;
                int outtimelength = dataItem.getBillingOutTime()!=null ? dataItem.getBillingOutTime().length() : 0;
                ...
                if(intimelength>0)
                {
                    dataRow.createCell(3).setCellValue(dataItem.getBillingInTime().substring(12,intimelength));
                }
                if(intimelength>0)
                {
                    dataRow.createCell(4).setCellValue(dataItem.getBillingOutTime().substring(12,intimelength));
                }
                dataRow.createCell(5).setCellValue(dataItem.getOAnumber());
                dataRow.createCell(6).setCellValue(dataItem.getTransportName());
                dataRow.createCell(7).setCellValue(dataItem.getMobileNumber());
                dataRow.createCell(8).setCellValue(dataItem.getGrossWeight());
                dataRow.createCell(9).setCellValue(dataItem.getTareWeight());
                dataRow.createCell(10).setCellValue(dataItem.getNetWeight());
                dataRow.createCell(11).setCellValue(dataItem.getSealNumber());
                dataRow.createCell(12).setCellValue(dataItem.getOutTotalQty());
                dataRow.createCell(13).setCellValue(dataItem.getInvoiceNumber());
                dataRow.createCell(14).setCellValue(dataItem.getBillingOutBatchNo());
                dataRow.createCell(15).setCellValue(dataItem.getBillingRemark());
            }
            // Save the workbook
    ...
    private void saveWorkbookToFile(HSSFWorkbook hssfWorkBook) {
        try {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.R) {
                ...
                int counter = 1;
                String fileName = "Outward Truck Out Billing Data_" + dateTimeSuffix + ".xls";
                File outputfile = new File(storageVolume.getDirectory().getPath() + "/Download/" + fileName);
                while (outputfile.exists()) {
                    counter++;
                    fileName = "Outward Truck Out Billing Data_" + dateTimeSuffix + "_" + counter + ".xls";
    ...
    {
        rvClub = findViewById(R.id.recyclerviewitin_billing_out_hcogrid_truckin);
        headerscroll = findViewById(R.id.itin_Billing_out_coheaderscroll_truckwe);
    }
    private void setUpRecyclerView()
    {
        adapterBillingOutComplete  = new Adapter_Billing_Out_Complete(clubList);
        FixedGridLayoutManager manager = new FixedGridLayoutManager();
        manager.setTotalColumnCount(1);
        rvClub.setLayoutManager(manager);
        rvClub.setAdapter(adapterBillingOutComplete);
    ...
    public void fetchDataFromApiforweigh(String FromDate,String Todate,String vehicleType,char nextprocess, char inOut) {
        Call<List<Common_Outward_model>> call = outwardTanker.get_tankerin_billing_completed(FromDate,Todate,vehicleType,nextprocess,inOut);
        call.enqueue(new Callback<List<Common_Outward_model>>() {
            ...
            public void onFailure(Call<List<Common_Outward_model>> call, Throwable t) {
                ...
                Toasty.error(Billing_Out_OR_Complete.this,"failed..!", Toast.LENGTH_SHORT).show();
            }
        });
